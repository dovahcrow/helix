...
pub fn cleanup(ctx: &mut Context) {
    let (view, doc) = current!(ctx.editor);
    view.dimmed = false;
    doc.in_visual_jump_mode = false;
    doc.visual_jump_labels[0].clear();
    doc.visual_jump_labels[1].clear();
    doc.visual_jump_labels[2].clear();
}
    ...
    F: FnOnce(&mut Context, KeyEvent) + 'static,
{
    ...
    doc.visual_jump_labels[0] = overlays_single.into();
    doc.visual_jump_labels[1] = overlays_multi_first.into();
    doc.visual_jump_labels[2] = overlays_multi_rest.into();
